...
ILI9486_SPI_18BIT::ILI9486_SPI_18BIT(uint8_t dc):ILI9486(480,320){
	cs = SS;
	this -> dc = dc;
...
void ILI9486_SPI_18BIT::init() {
    ...
    writeCom(0x11); // Sleep out, also SW reset
    delay(120);
...
void ILI9486_SPI_18BIT::setWindow ( Rectangle* r ) {
    uint16_t x1 = r -> getX(),
            y1 = r -> getY(),
            x2 = r -> getEndX(),
            y2 = r -> getEndY();
...
void ILI9486_SPI_18BIT::fillRect ( Rectangle* r ) {
	setWindow(r);
...
void ILI9486_SPI_18BIT::setRotate(uint8_t rotate) {
    rotate %= 4;
    if (getRotate()==rotate) {
		return;
	}
    ...
    case 3:
        writeCom ( 0x36, 0b01010000 ) ;
        break;
    }
}

void ILI9486_SPI_18BIT::fillShape(BufferScreen* buf) {
...
void ILI9486_SPI_18BIT::drawShape ( Dot *d ) {
...
ILI9486_SPI_18BIT::~ILI9486_SPI_18BIT() {
	spi -> endTransaction();
	spi -> end();
	delete spi ;
}
