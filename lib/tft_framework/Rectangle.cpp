/**
 * @file Rectangle.cpp
 * @brief Implementation of the Rectangle class for rectangular shapes.
 * 
 * Rectangle is the fundamental drawing primitive. All graphics operations
 * ultimately use fillRect() to render pixels to the display.
 */
...
void Rectangle::fill(Screen* scr) {
	...
	int x0 = tmp.getX(), y0 = tmp.getY(), x1 = tmp.getEndX(),
		y1 = tmp.getEndY(), maxX = scr->getWidth() - 1,
