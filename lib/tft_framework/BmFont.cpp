/**
 * @file BmFont.cpp
 * @brief Implementation of bitmap font loader with file-based font support.
 * 
 * Loads custom bitmap fonts from files, with support for variable-width
 * characters and an LRU cache for performance optimization.
 */
...
bool BmFont::isValid() { return valid; }
	...
	for (int i = 0; i < charCount; i++) {
		if (chars[i]->id == c) {
			...
			if (!selected->cache) {
				...
				selected->cache =
					new uint8_t[selected->width * selected->height];
				uint8_t* pointer = selected->cache;
...
uint8_t BmFont::getCacheSize() { return cacheSize; }