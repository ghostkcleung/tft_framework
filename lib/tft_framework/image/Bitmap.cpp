/**
 * @file Bitmap.cpp
 * @brief Implementation of BMP image file loader and renderer.
 * 
 * Loads Windows Bitmap (.bmp) files and renders them to the display.
 * Supports standard uncompressed 24-bit RGB BMP format.
 * Automatically converts RGB888 to RGB565 for display.
 */
...
void Bitmap::fillGeneric(Screen* scr) {
	...
	if (!sortRect(&window, this)) {
		return;
	}

	window.moveTo(*this);
	if (!sortRect(&window, scr)) {
		return;
	}
	...
	uint16_t winW = window.getWidth(), imgW = getWidth(),
			 bufSize = winW * bytePerPixel;

	int16_t winX = window.getX(), winY = window.getY(),
			winEndY = window.getEndY(), viewX = viewport.getX();
	...
	if (colorDepth > 15) {
		...
		File f = getFile();
		f.seek(dataOffset +
			   (getHeight() - window.getHeight() - viewport.getY()) * imgW *
				   bytePerPixel);
...
uint32_t Bitmap::getDataOffset() { return dataOffset; }