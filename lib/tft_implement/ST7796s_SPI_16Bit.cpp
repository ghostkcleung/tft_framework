...
void ST_7796s_SPI_16Bit::init() {
	...
	{
		uint8_t datas[] = {0x80, 0x02, 0x3B};

		writeComs(0xB6, datas, sizeof(datas));
	}

	{
		uint8_t datas[] = {0x40, 0x8A, 0x00, 0x00, 0x29, 0x19, 0xA5, 0x33};
	...
	{
		uint8_t datas[] = {0xF0, 0x09, 0x0B, 0x06, 0x04, 0x15, 0x2F,
						   0x54, 0x42, 0x3C, 0x17, 0x14, 0x18, 0x18};

		writeComs(0xE0, datas, sizeof(datas));
	}

	{
		uint8_t datas[] = {0xF0, 0x09, 0x0B, 0x06, 0x04, 0x03, 0x2B,
...
void ST_7796s_SPI_16Bit::fillShape(Bitmap* bmp) {
	if (bmp->getColorDepth() == 16) {
		...
		uint16_t viewH = viewport.getHeight(), viewW = viewport.getWidth(),
				 imgW = bmp->getWidth(), bufSize = viewW * 2;
		...
		f.seek(bmp->getDataOffset() +
			   (bmp->getHeight() - viewH - viewY) * imgW * 2);
