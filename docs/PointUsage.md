...
- **Not Drawable**: Use [Dot](DotUsage.md) to draw points on screen
...
To draw a point (pixel) on the screen, you must use the **Dot class**:
...
Screen* scr = new YourScreenImplementation();

// Wrong: Point cannot be drawn
Point p(100, 100);
// p.draw(scr);  // ERROR - Point has no draw() method

// Correct: Use Dot to draw a pixel
Dot dot;
dot.setPoint(100, 100);       // Position the dot
dot.setColor(255, 0, 0);      // Set color (red)
dot.draw(scr);                // Draw on screen

// Or create Dot with position
Dot dot2(150, 150);           // Dot inherits from Point
dot2.setColor(0, 255, 0);     // Green
dot2.draw(scr);
```

See [Dot Usage](DotUsage.md) for more information on drawing pixels.
...
- **Shape** class (along with Color) - for all drawable shapes
- **Image** class (along with Fillable) - for bitmap images

The `move()` method is declared `virtual`, allowing derived classes to override movement behavior if needed.

**Example:**
```cpp
// Dot inherits from Shape, which inherits from Point
Dot d(100, 100);
d.offsetX(10);                // Point's method
d.setColor(255, 0, 0);        // Color's method
d.draw(scr);                  // Shape's method (implemented in Dot)
```
...
for (int i = 0; i < 10; i++) {
    ...
    // Use position for something (not drawing Point directly!)
}
```
...
- **[Dot Usage](DotUsage.md)** - Draw single pixels on screen
- [Shape Class](shape.md) - Base class for drawable shapes
- [Line Usage](LineUsage.md) - Lines use two points
- [Rectangle Usage](RectangleUsage.md) - Uses point for position
